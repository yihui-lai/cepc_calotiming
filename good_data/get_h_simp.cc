...
void get_h_simp() {
    ...
    for(int k=0;k<nhst;k++) {
        for(int j=0;j<npoints;j++){
            ...
            if (k==1)
                CC_ceren(filenames_p[j],outname_p[j], aatruemean[j] ,&mean,&rms);
            rrmean[j][k]=mean/cali_ce[j];
            rrrms[j][k]=rms/cali_ce[j];
            rrres[j][k]=0;
            if(rrmean[j][k]!=0) rrres[j][k]=rrrms[j][k]/rrmean[j][k];
            
        }
    }
    for(int l=0;l<npoints;l++) {CC[l]=rrmean[l][1];CC_reso[l]=rrres[l][1];
        cout<<"CC[l]: "<<CC[l]<<endl;}
    ...
    for(int k=0;k<nhst;k++) {
        for(int j=0;j<npoints;j++){
            ...
            if (k==1)
                CC_Ana_p(filenames_p[j],outname_p[j], aatruemean[j] ,&mean,&rms);
            rrmean[j][k]=mean/cali_sc[j];
            rrrms[j][k]=rms/cali_sc[j];
            rrres[j][k]=0;
            if(rrmean[j][k]!=0) rrres[j][k]=rrrms[j][k]/rrmean[j][k];
            
        }
    }
    for(int l=0;l<npoints;l++) {SS[l]=rrmean[l][1];SS_reso[l]=rrres[l][1];
        cout<<"SS[l]: "<<SS[l]<<endl;}
    ...
    for(int k=0;k<nhst;k++) {
        for(int j=0;j<npoints;j++){
            ...
            if (k==1) CC_Fem(filenames_p[j],outname_p[j], aatruemean[j] ,&mean,&rms);
            rrmean[j][k]=mean;
            rrrms[j][k]=rms;
            rrres[j][k]=0;
    ...
    for(int l=0;l<npoints;l++) {
        Fem[l]=rrmean[l][1];Fem_reso[l]=rrres[l][1];
